...
class ImportExport extends CoreBase {
	constructor(registry) {
		...
		this.registry.on('io_connect', (client) => {
			...
			client.on('loadsave_reset_page_all', (page) => {
				this.page.getPage(page).reset()
			})

			client.on('loadsave_reset_page_nav', (page) => {
				this.page.getPage(page).resetNav()
			})
	...
	importAll(data) {
		...
		for (let page = 1; page <= 99; ++page) {
			if (data.page !== undefined && data.page[page] !== undefined) {
				this.page.setPageName(page, data.page[page].name, false)
			} else {
				this.page.setPageName(page, 'PAGE', false)
			}
	...
	importPage(topage, frompage, data) {
		...
		if (data.page !== undefined) {
			this.page.setPageName(topage, data.page.name)
		} else {
			this.page.setPageName(topage, 'PAGE')
		}
	...
	processHttpRequest(req, res, done) {
		...
		if ((match = req.url.match(/^\/page_export\/((\d+))?/))) {
			...
			exp.page = this.page.getPageSimple(page)
		...
		if ((match = req.url.match(/^\/full_export/))) {
			...
			exp.page = this.page.getAllSimple()
