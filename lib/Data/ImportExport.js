...
class DataImportExport extends CoreBase {
	...
	clientConnect(client) {
		...
		client.onPromise('loadsave:reset', this.#reset.bind(this))
	...
	async #reset(config) {
		...
		if (config && config.connections === true) {
			await this.instance.deleteAllInstances()
		}

		if (config && config.surfaces === true) {
			this.surfaces.reset()
		}

		if (config && config.triggers === true) {
			this.triggers.reset()
		}

		if (config && config.customVariables === true) {
			this.instance.variable.custom.reset()
		}

		if (config && config.userconfig === true) {
