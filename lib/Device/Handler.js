...
var Image = require('../Resources/Image')
...
class DeviceHandler {
	...
	drawPage() {
		if (!this.lockedout) {
			this.data = this.page.getPageImages(this.page)

			for (var i in this.data) {
				this.panel.draw(i, this.data[i].buffer)
			}
		} else {
			this.datap = this.getImagesForPincode(this.currentpin)
			...
			/* Draw blank image for other banks is not used
