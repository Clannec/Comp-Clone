	...
	system.on('feedback_check_bank', function (page, bank, constraint) {
		...
		if (self.feedbacks[page][bank] !== undefined) {
			...
			system.emit('get_bank', page, bank, function (bank_obj) {
				...
				for (var i in self.feedbacks[page][bank]) {
					let feedback = self.feedbacks[page][bank][i]
					...
					if (instance) {
						...
						try {
							if (definition !== undefined && definition.options !== undefined && definition.options.length > 0) {
								feedback = _.cloneDeep(feedback)

								for (const option in definition.options) {
									const opt = definition.options[option]
									if (
										opt.type == 'textinput' &&
										(opt.regex === undefined || opt.regex == '') &&
										opt.variableLookup === true &&
										opt.variableParse !== false
									) {
										self.system.emit('variable_parse', feedback.options[opt.id], function (_parsed) {
											debug(_parsed)
											feedback.options[opt.id] = _parsed
										})
									}
								}
							}
						} catch (e) {
							debug(`error parsing variables in options: ${e}`)
						}
